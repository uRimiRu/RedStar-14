...
using Content.Client._CorvaxGoob.SecApartment.Stylesheets;
...
public sealed partial class SquadEntry : PanelContainer
{
    ...
    private string GetStatusText(SquadStatus status)
    {
        ...
        {
            SquadStatus.Active => _loc.GetString("sec-apartment-active"),
            SquadStatus.OnBreak => _loc.GetString("sec-apartment-on-break"),
            _ => _loc.GetString("sec-apartment-unknown")
        };
    }

    private string GetIconText(SquadIconNum icon)
    {
        if (_loc.TryGetString($"sec-apartment-icon-{icon.ToString().ToLower()}", out var name))
            return name;
        else
            return _loc.GetString("sec-apartment-unknown");
    }
}
