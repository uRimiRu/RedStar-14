...
public sealed partial class SecApartmentWindow : BaseWindow
{
    ...
    public SecApartmentWindow()
    {
        ...
        CreateSquadButton.OnPressed += _ =>
        {
            if (!string.IsNullOrWhiteSpace(NewSquadName.Text))
            {
                ...
                var maxsquadName = _cfm.GetCVar(CCCVars.MaxSquadNameLength);

                if (squadName.Length > maxsquadName)
                    squadName = squadName[..maxsquadName];
    ...
    private void UpdateSquadsList(List<Squad> squads)
    {
        ...
        {
            var entry = new SquadEntry(squad, _styles, _prototypeManager, _sprite, _loc);
            ...
            entry.OnRenamePressed += newName =>
            {
                ...
                if (cleanName.Length > maxsquadName)
                    cleanName = cleanName[..maxsquadName];
