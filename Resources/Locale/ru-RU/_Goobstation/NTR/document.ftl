# templates
# service
ntr-document-service-starting-text1 = [color=#b8972d]███[/color][color=#1d7a1d]░███░░░░██░░░░[/color][color=#b8972d]                          ★ ★ ★[/color][color=#1d7a1d]
    ░██░████░░░██░░░░           [head=3]Бланк документа[/head]
    ░░█░██░██░░██░█░░                   [head=3]СССП[/head]
    ░░░░██░░██░██░██░                        [bold]ГШ-СРВ[/bold]
    ░░░░██░░░████░[/color][color=#b8972d]███[/color][color=#b8972d]                          ★ ★ ★[/color]
    ==================================================[bold]
    Время от начала смены: { $date }
# security
ntr-document-security-starting-text1 = [color=#b8972d]███[/color][color=#1d7a1d]░███░░░░██░░░░[/color][color=#b8972d]                          ★ ★ ★[/color][color=#1d7a1d]
    ░██░████░░░██░░░░           [head=3]Бланк документа[/head]
    ░░█░██░██░░██░█░░                   [head=3]СССП[/head]
    ░░░░██░░██░██░██░                        [bold]ГШ-СБ[/bold]
    ░░░░██░░░████░[/color][color=#b8972d]███[/color][color=#b8972d]                          ★ ★ ★[/color]
    ==================================================[bold]
    Время от начала смены: { $date }
# cargo
ntr-document-cargo-starting-text1 = [color=#b8972d]███[/color][color=#1d7a1d]░███░░░░██░░░░[/color][color=#b8972d]                          ★ ★ ★[/color][color=#1d7a1d]
    ░██░████░░░██░░░░           [head=3]Бланк документа[/head]
    ░░█░██░██░░██░█░░                   [head=3]СССП[/head]
    ░░░░██░░██░██░██░                        [bold]ГШ-СНБ[/bold]
    ░░░░██░░░████░[/color][color=#b8972d]███[/color][color=#b8972d]                          ★ ★ ★[/color]
    ==================================================[bold]
    Время от начала смены: { $date }
# medical
ntr-document-medical-starting-text1 = [color=#b8972d]███[/color][color=#1d7a1d]░███░░░░██░░░░[/color][color=#b8972d]                          ★ ★ ★[/color][color=#1d7a1d]
    ░██░████░░░██░░░░           [head=3]Бланк документа[/head]
    ░░█░██░██░░██░█░░                   [head=3]СССП[/head]
    ░░░░██░░██░██░██░                        [bold]ГШ-МЕД[/bold]
    ░░░░██░░░████░[/color][color=#b8972d]███[/color][color=#b8972d]                          ★ ★ ★[/color]
    ==================================================[bold]
    Время от начала смены: { $date }
# engineering
ntr-document-engineering-starting-text1 = [color=#b8972d]███[/color][color=#1d7a1d]░███░░░░██░░░░[/color][color=#b8972d]                          ★ ★ ★[/color][color=#1d7a1d]
    ░██░████░░░██░░░░           [head=3]Бланк документа[/head]
    ░░█░██░██░░██░█░░                   [head=3]СССП[/head]
    ░░░░██░░██░██░██░                        [bold]ГШ-ИНЖ[/bold]
    ░░░░██░░░████░[/color][color=#b8972d]███[/color][color=#b8972d]                          ★ ★ ★[/color]
    ==================================================[bold]
    Время от начала смены: { $date }
# science
ntr-document-science-starting-text1 = [color=#b8972d]███[/color][color=#1d7a1d]░███░░░░██░░░░[/color][color=#b8972d]                          ★ ★ ★[/color][color=#1d7a1d]
    ░██░████░░░██░░░░           [head=3]Бланк документа[/head]
    ░░█░██░██░░██░█░░                   [head=3]СССП[/head]
    ░░░░██░░██░██░██░                        [bold]ГШ-НИО[/bold]
    ░░░░██░░░████░[/color][color=#b8972d]███[/color][color=#b8972d]                          ★ ★ ★[/color]
    ==================================================[bold]
    Время от начала смены: { $date }
ntr-document-service-document-text =
    { $start }
    СССП хочет, чтобы вы знали, что вы не { $text1 } { $text2 }
    СССП будет удовлетворена, если вы { $text3 }
    Подписи ниже подтверждают, что { $text4 }
ntr-document-security-document-text =
    { $start }
    СССП хочет, чтобы вы проверили некоторые вещи перед тем, как поставить печать, убедитесь, что { $text1 } { $text2 }
    { $text3 }
    { $text4 }
ntr-document-cargo-document-text =
    { $start }
    { $text1 }
    { $text2 }
    Ставя здесь печать, вы { $text3 }
ntr-document-medical-document-text =
    { $start }
    { $text1 } { $text2 }
    { $text3 }
    Ставя здесь печать, вы { $text4 }
ntr-document-engineering-document-text =
    { $start }
    { $text1 } { $text2 }
    { $text3 }
    Ставя здесь печать, вы { $text4 }
ntr-document-science-document-text =
    { $start }
    Мы внимательно наблюдаем за Научно-исследовательским отделом. { $text1 } { $text2 }
    В связи со всем вышеизложенным, мы хотим, чтобы вы убедились, что { $text3 }
    Печати ниже подтверждают, что { $text4 }
